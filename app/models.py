...
class Movie(db.Model):
    ...
    backdrop_path = db.Column(db.String(64), nullable=False)
    ...
    def get_genres_string(self):
        genre_str = ''
        for i, movie_genre in enumerate(self.genres):
            if i != 0:
                genre_str += str(movie_genre).lower() + ', '
            elif i == len(self.genres) - 1:
                genre_str += str(movie_genre).lower()
            else:
                genre_str += str(movie_genre)
